...
export PATH="${PATH}:/sbin:/usr/sbin:/usr/local/sbin:/usr/sbin"
export LC_ALL=C
...
if [ "${SEND_WEBHOOK}" = "YES" ]; then
    # Send a webhook notification using curl
    curl -X POST "${WEBHOOK_URL}" \
        ...
        -d "alarm_id=${alarm_id}" \
        -d "duration=${duration}" \
        -d "duration_txt=${duration_txt}" \
        -d "severity=${severity}" \
        -d "raised_for=${raised_for}" \
        -d "status_message=${status_message}" \
        -d "url_host=${url_host}" \
        -d "url_name=${url_name}" \
        -d "url_chart=${url_chart}" \
        -d "non_clear_duration=${non_clear_duration}" \
        -d "non_clear_duration_txt=${non_clear_duration_txt}" \
        -d "event_id=${event_id}" \
        -d "when=${when}" \
        -d "old_status=${old_status}" \
        -d "source=${source}" \
        -d "value_string=${value_string}" \
        -d "user_id=USER_ID" \
